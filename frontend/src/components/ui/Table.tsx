...
export function Table<T>({
  ...
  if (loading) {
    return (
      <div className="overflow-x-auto rounded-card border border-border/70">
        <table className="w-full text-body border-collapse">
          <caption className="sr-only">{caption ?? "Tableau"} — chargement en cours</caption>
          <tbody>
            {Array.from({ length: 5 }).map((_, i) => (
              <tr key={i} className="border-b border-border/60 last:border-0">
                {columns.map((col) => (
                  <td key={col.key} className={cellPadding}>
                    <div className="h-4 rounded bg-muted animate-pulse" style={{ width: `${55 + ((i * 13 + col.key.length * 7) % 35)}%` }} />
                  </td>
                ))}
              </tr>
            ))}
    ...
    <div className="space-y-2">
      <div className="overflow-x-auto rounded-card border border-border/70">
        <table className="w-full text-body border-collapse">
          ...
          <thead className="bg-muted/40 text-muted-foreground text-overline uppercase">
            <tr>
              {selectable && (
              ...
              {columns.map((col) => {
                ...
                const stickyClass = col.sticky ? "sticky left-0 z-10 bg-muted/95" : "";
                return (
                  <th
                    key={col.key}
                    scope="col"
          ...
          <tbody className="divide-y divide-border/60">
            {pagedRows.map((row) => {
              ...
              const rowBg = highlighted ? "bg-primary/5" : rowSelected ? "bg-accent/50" : undefined;
              return (
                <tr key={key} className={rowBg}>
                  ...
                  {columns.map((col) => {
                    ...
                    return (
                      <td
                        key={col.key}
                        className={`${cellPadding} text-foreground/90 whitespace-nowrap ${ALIGN_CLASSES[col.align ?? "left"]} ${stickyClass} ${col.className ?? ""}`}
                      >
        ...
        <div className="flex items-center justify-between text-caption text-muted-foreground px-1">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="rounded-control p-1 border border-border disabled:opacity-40 disabled:cursor-not-allowed hover:bg-muted transition-colors"
            >
              <ChevronLeft size={14} />
            </button>
            <button
