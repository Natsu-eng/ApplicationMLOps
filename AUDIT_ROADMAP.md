...
| **Computer Vision** | **Backend + frontend des 4 sous-lots livrés (106 tests)** ; legacy substantiel mais bugué | ~11 300 lignes de logique PyTorch legacy (classification + transfer learning + détection d'anomalies MVTec AD avec autoencodeurs/PatchCore/Siamese + Grad-CAM), documentée "Production-Ready" par l'app elle-même mais avec **18 bugs déjà recensés dans 4 audits legacy existants** (`docs/legacy/`), dont 9 critiques — **tous corrigés pendant le portage** (masque binaire désormais généré, heatmap systématiquement calculée, seuil calibré par J de Youden, alignement dimensionnel testé). Module backend neuf (`services/vision_*.py`), aucune réutilisation du state global Streamlit. Découpage en 4 sous-lots (A upload/dataset → B classification/transfer learning → C anomalies visuelles MVTec AD → D Grad-CAM), livrés bout en bout (backend + pages React). Testé avec l'app réelle : plusieurs bugs corrigés (structure MVTec AD officielle, proxy Vite, endpoint image manquant, chemins Windows). Détection d'objets/annotation assistée hors périmètre, reportées à un Lot 16+ non cadré. |
...
- [x] **Lot 13** — Réduction de dimension **livré** (2026-08-15) : PCA/t-SNE/UMAP (`umap-learn==0.5.6`, épinglé volontairement pour éviter une mise à niveau majeure de scikit-learn), module séparé (`dimensionality_registry.py`/`dimensionality_training.py`), `DimensionalityJob`/`DimensionalityModel`/`DimensionalityPoint`, `workers/dimensionality_worker.py`, router `api/routers/dimensionality.py`, page `pages/DimensionalityReduction.tsx`. PCA toujours calculée en plus de la méthode choisie (variance expliquée/loadings/repère de fidélité), jamais de leaderboard façon clustering (pas de métrique de qualité commune entre familles). Lien croisé (query params, pas de couplage backend) depuis les résultats de clustering. 34 tests.
- [x] **Lot 14** — Détection d'anomalies tabulaire **livrée** (2026-08-15) : Isolation Forest + LOF toujours évalués ensemble (jamais de "gagnant" élu, pas de vérité terrain disponible), score de consensus (moyenne des rangs percentiles) et drapeau `agreement`, module séparé (`anomaly_registry.py`/`anomaly_training.py`), `AnomalyJob`/`AnomalyModel`/`AnomalyObservationRecord`, `workers/anomaly_worker.py`, router `api/routers/anomalies.py`, page `pages/AnomalyDetection.tsx`. Quota désormais partagé par les 4 types de job via un helper commun (`services/job_quota.py`, extrait à cette occasion — corrige au passage un oubli du Lot 11+12 où `training.py` ne comptait jamais les jobs de clustering actifs). 30 tests. Les 3 modules du pilier "ML non supervisé" sont désormais actifs.
- [ ] **Lot 15 — Computer Vision** : portage des architectures PyTorch legacy vers un module backend neuf, en corrigeant au passage les 9 bugs critiques déjà documentés dans `docs/legacy/`. Découpé en 4 sous-lots :
  - [x] **Sous-lot A — Infrastructure dataset image** (2026-08-15) : upload ZIP synchrone (`services/vision_datasets.py`, pure/testable), détection stricte de structure — dossiers de classes (classification) ou `train/good`+`test/good`+`test/<defaut>` (MVTec AD), jamais devinée en silence (correctif préventif du bug #1 — labels `y_train` mal validés). Validation systématique : images corrompues (exclues, jamais bloquant), doublons (hash SHA-256), sous-dimensionnées (<20px), classes déséquilibrées (avertissement, ratio >10x). Protection zip-slip et zip-bombe (cap taille décompressée + nombre d'images, `max_vision_dataset_images`). Table `VisionDataset`, router `api/routers/vision_datasets.py` (`POST/GET/DELETE /vision/datasets`), stockage `storage/datasets_vision/{organization_id}/{dataset_id}/`. `Pillow` épinglé en dépendance directe (était transitive). 22 tests ciblés verts (service + API).
  - [x] **Sous-lot B — Classification d'images + transfer learning, backend** (2026-08-16) : `torch`/`torchvision` CPU ajoutés (`--extra-index-url` PyTorch CPU dans `requirements.txt`, jamais de roue CUDA). 2 backbones seulement (`mobilenet_v3_small` par défaut, `resnet18`) — volontairement pas les 17 du legacy, impraticables en CPU. Backbone gelé par défaut (transfer learning strict), dégel optionnel à une époque donnée. Garde-fou de temps interne (1500s, sous le timeout RQ 1800s) plutôt qu'un timeout brutal sur un gros dataset. Tables `VisionClassificationJob`/`VisionClassificationModel`, router `api/routers/vision_classification.py`. **Bug réel trouvé et corrigé en testant** : le dossier de stockage d'un dataset vision (sous-lot A) n'était pas nettoyé avant extraction — un id réutilisé (observé en test SQLite) héritait silencieusement des fichiers d'un dataset précédent. `services/job_quota.py` simplifié à cette occasion (`ALL_JOB_MODELS`, constante unique remplaçant la liste recopiée dans 4 routers — risque déjà matérialisé une fois au Lot 14). 30 tests verts (entraînement réel non mocké). **Frontend livré** (pages/VisionClassification.tsx, réutilisée pour B/D).
  - [x] **Sous-lot C — Détection d'anomalies visuelles MVTec AD, backend** (2026-08-16) : `services/vision_localization.py` (nouveau, réutilisable au sous-lot D) — `generate_binary_mask` (bug #14, fonction absente du legacy), `resize_map_to_original` (bugs #10/#17, réalignement systématique testé), `encode_heatmap_png`/`encode_mask_png` (PNG base64 directement affichables). `ConvAutoEncoder` entièrement convolutif, `IMAGE_SIZE` fixe imposé par le pipeline (pas de resize interne au modèle — évite la racine du bug #5). Entraînement sur `train/good/` uniquement, pas de normalisation ImageNet (évite le bug #11). Carte d'erreur calculée pour CHAQUE image de test systématiquement (bugs #8/#16). Seuil calibré par J de Youden sur `test/` (bugs #7/#12), vérifié par test que deux datasets donnent deux seuils différents. Tables `VisionAnomalyJob`/`VisionAnomalyModel`/`VisionAnomalyExampleRecord`, router `api/routers/vision_anomalies.py`. `services/job_quota.py` : `VisionAnomalyJob` ajouté à `ALL_JOB_MODELS` — un seul endroit modifié grâce au refactor du sous-lot B. 32 tests verts (entraînement réel non mocké). **Frontend livré** (voir sous-lot B pour classification, pages/VisionAnomalies.tsx pour ce sous-lot).
  - [x] **Sous-lot D — Grad-CAM, backend** (2026-08-16) : `gradcam_target_layer` ajouté au registre de classification (couche convolutive finale par backbone), `services/vision_gradcam.py` (nouveau), réutilise intégralement `services/vision_localization.py` du sous-lot C. Endpoint synchrone `POST /vision/classification/jobs/{id}/explain` (même principe que `POST /training/jobs/{id}/predict`). **Bug réel trouvé et corrigé en testant** : avec un backbone gelé (mode par défaut), le graphe autograd n'était pas connecté jusqu'à la couche cible (gradient vide/incorrect, silencieux) — corrigé par `input_tensor.requires_grad_(True)`, vérifié par un test dédié (heatmap non dégénérée). 11 tests verts. **Frontend livré** (voir sous-lot B pour classification, pages/VisionAnomalies.tsx pour ce sous-lot).
  - **Backend des 4 sous-lots terminé** (97 tests). **Aucune page frontend construite pour le pilier Vision** — prochain chantier à cadrer.
  - Hors périmètre confirmé (2026-08-15) : détection d'objets (bounding boxes) et annotation assistée → **Lot 16+**, non cadré, à ne pas oublier.
