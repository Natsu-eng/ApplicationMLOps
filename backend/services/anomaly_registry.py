...
class AnomalySpec:
    ...
    label: str
    family: Family
    build_estimator: Callable[[int, int], BaseEstimator]  # (n_samples, seed) -> estimateur


def _build_isolation_forest(n_samples: int, seed: int) -> BaseEstimator:
    # contamination="auto" (seuil dérivé de la formule du papier original,
    # pas une fraction arbitraire fixée à l'aveugle) — cohérent avec le mode
    # guidé du reste du produit (pas de paramètre à régler par défaut).
    return IsolationForest(contamination="auto", random_state=seed, n_estimators=200)


def _build_lof(n_samples: int, seed: int) -> BaseEstimator:
    ...
    return LocalOutlierFactor(contamination="auto", novelty=False, n_neighbors=n_neighbors)
