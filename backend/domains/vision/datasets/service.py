...
uniforme (zip, tar, dossier) — chacune la SEULE responsable de son format,
jamais mélangée au reste du pipeline.

Découpage (fichier de 822 lignes) : toute la couche « octets reçus →
liste de `_ExtractedMember` » vit désormais dans
`archive_extraction.py` — extraction zip/tar/dossier ET les défenses de
sécurité associées (zip-slip, liens tar, bombe de décompression, sniff du
format réel), regroupées pour être auditables d'un seul tenant. Ce module
garde ce qui vient APRÈS : détection de structure, validation/copie des
images, déduplication, EDA. Il reste le point d'entrée unique du domaine
pour les appelants (voir `__all__`).
"""
...
from domains.vision.datasets.archive_extraction import (
    UnsupportedFileType,
    VisionDatasetError,
    _extract_archive_members,
    _ExtractedMember,
    _members_from_uploaded_files,
    validate_archive_extension,
)

# `UnsupportedFileType` et `validate_archive_extension` ne sont pas utilisés
# dans ce module : ils sont ré-exportés délibérément (voir `__all__`) pour
# que `service.py` reste le point d'entrée unique du domaine côté appelants
# (routeur, tests), inchangé par le découpage interne.
__all__ = [
    "CLASS_IMBALANCE_WARNING_RATIO",
