...
de classification) avec `ANOMALY_IMAGE_SIZE` (128, taille d'entrée de
l'autoencodeur, réexportée localement sous le nom `IMAGE_SIZE` par
`domains/vision/anomalies/services/registry.py`) — deux constantes
légitimement différentes, chacune propre à son propre modèle, pas un
doublon à fusionner (vérifié explicitement lors de l'extraction). Toutes
deux vivent ici, jamais l'une chez l'autre sous-domaine : c'est ce qui
permet à `vision/datasets/router.py` (qui a besoin des deux) de ne
jamais dépendre directement de `vision.classification`/`vision.anomalies`
— seulement de ce module, commun aux trois."""
...
# Taille d'entrée de l'autoencodeur convolutif (pilier anomalies visuelles)
# — fixe, la responsabilité du redimensionnement est entièrement dans le
# pipeline de données, jamais dans le modèle (voir
# `domains/vision/anomalies/services/registry.py`, correctif du bug #5
