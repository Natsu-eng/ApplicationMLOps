"""Tests de `GET /anomalies/jobs/{id}/drift` — voir
...
from api.core.models import AnomalyScoreLog
...
def _create_and_complete_job(client, headers, dataset_id):
    with patch("domains.anomalies.router.analysis_queue") as mock_queue:
        ...
        job = client.post(
            "/api/anomalies/jobs", headers=headers, json={"dataset_id": dataset_id, "feature_columns": ["x1", "x2"]}
        ).json()
    run_anomaly_job(job["id"])
    return job


def _insert_logs(db, anomaly_job_id: int, organization_id: int, x1_values: list[float]) -> None:
    for x1 in x1_values:
...
def test_drift_409_when_no_result_yet(client):
    ...
    resp = client.get(f"/api/anomalies/jobs/{job['id']}/drift", headers=headers)
...
def test_predict_endpoint_persists_a_drift_log_entry(client, db_session):
    ...
    resp = client.post(f"/api/anomalies/jobs/{job['id']}/predict", headers=headers, json={"data": {"x1": 50, "x2": 20}})
    assert resp.status_code == 200

    count = db_session.query(AnomalyScoreLog).filter(AnomalyScoreLog.anomaly_job_id == job["id"]).count()
    assert count == 1
...
def test_drift_404_for_other_organization(client, db_session):
    ...
    resp = client.get(f"/api/anomalies/jobs/{job['id']}/drift", headers=headers_b)
