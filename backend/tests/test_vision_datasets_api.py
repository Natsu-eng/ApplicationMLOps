...
def test_upload_rejects_unsupported_extension(client):
    ...
    headers = _register(client)
    resp = client.post(
        "/vision/datasets", headers=headers, files={"files": ("dataset.png", io.BytesIO(b"x"), "image/png")}
    )
...
def test_upload_of_a_tar_gz_archive_is_accepted(client):
    ...
    resp = client.post(
        "/vision/datasets", headers=_register(client),
...
def test_upload_of_a_folder_multiple_files_is_accepted(client):
    ...
    resp = client.post("/vision/datasets", headers=_register(client), files=files)
    assert resp.status_code == 201
    body = resp.json()
    assert body["status"] == "ready"
...
def test_upload_rejects_empty_archive(client):
    headers = _register(client)
    resp = client.post(
        "/vision/datasets", headers=headers, files={"files": ("dataset.zip", io.BytesIO(b""), "application/zip")}
    )
    assert resp.status_code == 400
