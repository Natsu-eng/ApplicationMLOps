"""Tests de `GET /clustering/jobs/{id}/drift` — même logique PSI que
`test_drift_api.py` (pilier tabulaire supervisé), adaptée au clustering.
Entraînement réel (worker appelé directement, pas de mock), même
convention que `test_clustering_api.py`."""
...
from api.core.models import ClusterPredictionLog
...
def _create_and_complete_job(client, headers, dataset_id):
    with patch("domains.clustering.router.analysis_queue") as mock_queue:
        ...
        job = client.post(
            "/api/clustering/jobs",
            headers=headers,
            json={"dataset_id": dataset_id, "feature_columns": ["x1", "x2"], "algorithm_ids": ["kmeans"]},
        ).json()
    run_clustering_job(job["id"])
    return job


def _insert_logs(db, clustering_job_id: int, organization_id: int, x1_values: list[float]) -> None:
    for x1 in x1_values:
...
def test_drift_409_when_no_result_yet(client):
    ...
    resp = client.get(f"/api/clustering/jobs/{job['id']}/drift", headers=headers)
    assert resp.status_code == 409
    assert resp.json()["detail"]["code"] == "RESULTAT_INDISPONIBLE"


def test_predict_endpoint_persists_a_drift_log_entry(client, db_session):
    """La journalisation reelle (pas seulement testee en isolation via
    insertion directe) — un /predict reussi doit produire une ligne dans
    ClusterPredictionLog."""
    headers = _register(client)
    dataset = _upload_dataset(client, headers)
    job = _create_and_complete_job(client, headers, dataset["id"])
    db_session.expire_all()

    resp = client.post(f"/api/clustering/jobs/{job['id']}/predict", headers=headers, json={"data": {"x1": 50, "x2": 20}})
    assert resp.status_code == 200

    count = db_session.query(ClusterPredictionLog).filter(ClusterPredictionLog.clustering_job_id == job["id"]).count()
    assert count == 1
...
def test_drift_detects_a_shifted_feature(client, db_session):
    ...
    job = _create_and_complete_job(client, headers, dataset["id"])
    db_session.expire_all()
    shifted = list(_RNG.normal(150, 5, 100))  # dataset a x1 ~ N(50, 10)
...
def test_drift_404_for_other_organization(client, db_session):
    ...
    resp = client.get(f"/api/clustering/jobs/{job['id']}/drift", headers=headers_b)
