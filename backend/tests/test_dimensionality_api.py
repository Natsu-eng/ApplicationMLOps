...
def test_cancel_queued_job_marks_it_cancelled_and_keeps_history(client):
    ...
    resp = client.post(f"/api/dimensionality/jobs/{job['id']}/cancel", headers=headers)
    assert resp.status_code == 200
    assert resp.json()["status"] == "cancelled"
    assert client.get(f"/api/dimensionality/jobs/{job['id']}", headers=headers).json()["status"] == "cancelled"
...
def test_events_stream_closes_immediately_on_terminal_job(client):
    ...
    dataset = _upload_dataset(client, headers)
    job = _create_job(client, headers, dataset["id"]).json()
    client.post(f"/api/dimensionality/jobs/{job['id']}/cancel", headers=headers)

    resp = client.get(f"/api/dimensionality/jobs/{job['id']}/events", headers=headers)
    assert resp.status_code == 200
...
def test_events_stream_404_for_other_organization(client):
    ...
    headers_b = _register(client, "b@bureau-b.fr", "Bureau B")
    resp = client.get(f"/api/dimensionality/jobs/{job['id']}/events", headers=headers_b)
    assert resp.status_code == 404
...
def test_cancel_404_for_other_organization(client):
    ...
    headers_b = _register(client, "b@bureau-b.fr", "Bureau B")
    resp = client.post(f"/api/dimensionality/jobs/{job['id']}/cancel", headers=headers_b)
    assert resp.status_code == 404
...
def test_rerun_404_for_other_organization(client):
    ...
    headers_b = _register(client, "b@bureau-b.fr", "Bureau B")
    resp = client.post(f"/api/dimensionality/jobs/{job['id']}/rerun", headers=headers_b)
    assert resp.status_code == 404
