...
def _create_job(client, headers, dataset_id, **overrides):
    ...
    with patch("api.routers.clustering.training_queue") as mock_queue:
        ...
        return client.post("/clustering/jobs", headers=headers, json=body)


def test_algorithms_catalog_lists_registry(client):
    headers = _register(client)
    resp = client.get("/clustering/algorithms-catalog", headers=headers)
...
def test_list_jobs_isolated_between_organizations(client):
    ...
    resp = client.get("/clustering/jobs", headers=headers_b)
    assert resp.status_code == 200
    assert resp.json() == []

    resp_a = client.get("/clustering/jobs", headers=headers_a)
...
def test_get_job_404_for_other_organization(client):
    ...
    headers_b = _register(client, "b@bureau-b.fr", "Bureau B")
    resp = client.get(f"/clustering/jobs/{job['id']}", headers=headers_b)
    assert resp.status_code == 404
...
def test_result_endpoint_404_before_completion(client):
    ...
    resp = client.get(f"/clustering/jobs/{job['id']}/result", headers=headers)
    assert resp.status_code == 404
...
def test_delete_job_removes_it_from_history(client):
    ...
    resp = client.delete(f"/clustering/jobs/{job['id']}", headers=headers)
    assert resp.status_code == 204
    assert client.get(f"/clustering/jobs/{job['id']}", headers=headers).status_code == 404
